...
if [[ -f "$SCRIPT_DIR/cf.sh" ]]; then
  ...
  yan_download "$REPO_RAW_BASE/cf.sh" /usr/local/bin/yanstunnel-cf
...
declare -A commands=(
  [addhost]=add-host.sh [add-ws]=add-ws.sh [add-ssws]=add-ssws.sh [add-socks]=add-socks.sh
  [add-vless]=add-vless.sh [add-tr]=add-tr.sh [add-trgo]=add-trgo.sh
  [autoreboot]=autoreboot.sh [backup]=backup.sh [bbr]=bbr.sh
  [cek-bandwidth]=cek-bandwidth.sh [cek-trafik]=cek-trafik.sh
  [clearlog]=clearlog.sh [dns]=dns.sh [limit-speed]=limit-speed.sh [cek-ram]=ram.sh
  [netf]=netf.sh [restart]=restart.sh [restore]=restore.sh
  [running]=running.sh [tendang]=tendang.sh [usernew]=usernew.sh
  [webmin]=webmin.sh [xp]=xp.sh [ins-xray]=ins-xray.sh [ssh-vpn]=ssh-vpn.sh
