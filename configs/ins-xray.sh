#!/usr/bin/env bash
if [[ -r /usr/bin/yanbhoikfost/common ]]; then
    # shellcheck disable=SC1091
    source /usr/bin/yanbhoikfost/common
fi
# =========================================
...
# // Exporting URL Host
export Server_URL="raw.githubusercontent.com/yansyntax/yanstunnel/main/test"
export Server1_URL="raw.githubusercontent.com/yansyntax/yanstunnel/main/limit"
...
command -v yan_detect_os >/dev/null 2>&1 && yan_detect_os
command -v yan_detect_arch >/dev/null 2>&1 && yan_detect_arch

# // Exporting IP Address
export IP="$(yan_public_ip 2>/dev/null || true)"
...
clear
source /var/lib/scrz-prem/ipvps.conf 2>/dev/null || true
domain="${IP:-}"
[[ -r /etc/xray/domain ]] && domain="$(cat /etc/xray/domain)"
[[ -z "$domain" && -r /root/domain ]] && domain="$(cat /root/domain)"
[[ -n "$domain" ]] || { echo "Domain belum disiapkan."; exit 1; }

echo -e "[ ${GREEN}INFO${NC} ] Checking... "
sleep 1
echo -e "[ ${GREEN}INFO${NC} ] Preparing modern dependencies..."
apt-get update -y
for package in iptables iptables-persistent curl socat xz-utils wget dnsutils cron bash-completion openssl unzip jq chrony nginx certbot python3; do
    apt-cache show "$package" >/dev/null 2>&1 && apt-get install -y "$package" || true
done
timedatectl set-ntp true 2>/dev/null || true
timedatectl set-timezone "${YAN_TIMEZONE:-Asia/Jakarta}" 2>/dev/null || true
...
# Install the distribution package only; compiling a second copy breaks on
# newer Debian/Ubuntu toolchains.
apt-cache show wondershaper >/dev/null 2>&1 && apt-get install -y wondershaper || true
echo > /home/limit
...
wget -q -O /etc/nginx/nginx.conf "https://raw.githubusercontent.com/yansyntax/yanstunnel/main/configs/nginx.conf" 
#mkdir -p /home/vps/public_html
wget -q -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/yansyntax/yanstunnel/main/configs/vps.conf"

# Install Xray #
#==========#
# / / Ambil Xray Core Version Terbaru
latest_version="$(curl -fsSL https://api.github.com/repos/XTLS/Xray-core/releases/latest | jq -r .tag_name | sed 's/^v//')"
# / / Installation Xray Core
if [[ "${YAN_ARCH:-amd64}" == "arm64-v8a" ]]; then
    xray_asset="xray-linux-arm64-v8a.zip"
else
    xray_asset="xray-linux-64.zip"
fi
xraycore_link="https://github.com/XTLS/Xray-core/releases/download/v$latest_version/$xray_asset"
# / / Ambil Xray Core Version Terbaru
